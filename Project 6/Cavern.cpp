/*
  ...
  CSCI235 - Spring 2024 
  April 18, 2024 
  Cavern.cpp - implementation for Cavern class
*/
...
void Cavern::initializeAlienStack()
{
  ...
  for(int i = 0; i < size; i++)
  {
    if(items_[i]->getLevel() > max && items_[i]->getCategory() == "ALIEN")
    {
...
void Cavern::initializeUndeadStack()
{
  ...
  for(int i = 0; i < size; i++)
  {
    if(items_[i]->getLevel() > max && items_[i]->getCategory() == "UNDEAD")
    {
      max = items_[i]->getLevel();
    }
  }
...
void Cavern::initializeMysticalStack()
{
  ...
  for(int i = 0; i < size; i++)
  {
    if(items_[i]->getLevel() > max && items_[i]->getCategory() == "MYSTICAL")
    {
      max = items_[i]->getLevel();
    }
  }
...
void Cavern::initializeUnknownStack()
{
  ...
  for(int i = 0; i < size; i++)
  {
    if(items_[i]->getLevel() > max && items_[i]->getCategory() == "UNKNOWN")
    {
      max = items_[i]->getLevel();
    }
  }
...
/**
  ...
  *        1: [ATTACK 1 NAME]\t\t2: [ATTACK 2 NAME]\t\t3: [ATTACK 3 NAME]
*/
void Cavern::setAttacks(std::stack<Creature*> creature_ptr_stack)
{
  std::stack<Creature*> temp = creature_ptr_stack; 
  if (!temp.empty()) 
  { 
    Creature* moo = temp.top(); 
    temp.pop(); 
    //clear stacks
    if(moo->getCategory() == "ALIEN") 
    { 
      clearAlienStack(); 
    } 
    else if(moo->getCategory() == "UNDEAD") 
    { 
      clearUndeadStack(); 
    } 
    else if(moo->getCategory() == "MYSTICAL") 
    { 
      clearMysticalStack(); 
    } 
    else if(moo->getCategory() == "UNKNOWN") 
    {
      clearUnknownStack(); 
    } 
    temp.push(moo); //push again

  } 
  
  std::stack<Creature*> reverse; //new stack to reverse
  while(!temp.empty()) 
  { 
    //loop through the temporary stack 
    Creature* bop = temp.top(); 
    temp.pop(); 
    std::cout << "SELECT 2 ATTACKS FOR " << bop->getName() << std::endl; 
  ...
  while(!reverse.empty()) 
  { // rebuild the Cavern's appropriate stack based on the creatures' categories 
    Creature* bop = reverse.top(); 
    reverse.pop(); 
    if (bop->getCategory() == "ALIEN") 
    {
      alien_stack_.push(bop); 
    } 
    else if (bop->getCategory() == "UNDEAD") 
    { 
      undead_stack_.push(bop); 
    } 
    else if (bop->getCategory() == "MYSTICAL") 
    {
      mystical_stack_.push(bop); 
    } 
    else if (bop->getCategory() == "UNKNOWN") 
    { 
      unknown_stack_.push(bop); 
    }
  }  
}
