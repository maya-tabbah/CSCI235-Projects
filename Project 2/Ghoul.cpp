...
Ghoul::Ghoul(): Creature(), level_of_decay_{0}, faction_{NONE}
{
    setCategory(UNDEAD); //ADD UNDEAD
} 
...
*/
Ghoul::Ghoul(const std::string &name, Category category, int hitpoints, int level, bool tame, int level_of_decay, Faction faction, bool can_transform):Creature(name, category, hitpoints, level,tame)
{
    (level_of_decay >= 0) ? level_of_decay_ = level_of_decay: level_of_decay_ = 0;
    setFaction(faction); //checks valid faction 
